...
def test_sign_and_verify_accept_bytes_like(
    signer: falcon1024.Signer, verifier: falcon1024.Verifier
) -> None:
...
def test_generate_accepts_bytes_like_seed() -> None:
    a = falcon1024.Signer.generate(seed=bytearray(range(32)))
    b = falcon1024.Signer.generate(seed=memoryview(bytes(range(32))))
    c = falcon1024.Signer.generate(seed=bytes(range(32)))
    assert a.public_key == b.public_key == c.public_key


def test_constructors_accept_bytes_like(signer: falcon1024.Signer) -> None:
    v = falcon1024.Verifier(bytearray(signer.public_key))
    assert v.public_key == signer.public_key
    s = falcon1024.Signer(bytearray(signer.private_key))
    assert s.public_key == signer.public_key
    assert falcon1024.Signer(memoryview(signer.private_key)).public_key == v.public_key


def test_constructor_stores_the_key_the_public_key_was_derived_from(
    signer: falcon1024.Signer, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    theirs = falcon1024.Signer.generate(seed=b"substituted").private_key
...
def test_int_is_rejected_at_every_entry_point(
    signer: falcon1024.Signer, verifier: falcon1024.Verifier, name: str
) -> None:
    ...
    with pytest.raises(TypeError, match=name):
        if name == "seed":
            falcon1024.Signer.generate(seed=5)  # type: ignore[arg-type]
        elif name == "message":
