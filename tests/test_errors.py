...
def test_generate_rejects_empty_seed() -> None:
    ...
    with pytest.raises(ValueError):
        falcon1024.Signer.generate(seed=b"")
...
def test_generate_accepts_any_non_empty_seed_length(seed: bytes) -> None:
    ...
    signer = falcon1024.Signer.generate(seed=seed)
    assert len(signer.public_key) == falcon1024.PUBLIC_KEY_SIZE
...
def test_signer_rejects_undecodable_private_key() -> None:
    ...
    with pytest.raises(fp.KeygenError):
        falcon1024.Signer(bytes(falcon1024.PRIVATE_KEY_SIZE))
...
def test_signing_rejects_a_private_key_that_construction_accepts() -> None:
    ...
    key = bytearray(falcon1024.Signer.generate(seed=b"corrupt-F").private_key)
    key[-1] ^= 0x01
    signer = falcon1024.Signer(bytes(key))
